...
import (
	...
	"github.com/baranyildirim/helmvendor/pkg/manifest"
...
type Helm interface {
	...
	ChartExists(chart string, opts *VendorOpts) (string, error)
}
...
func (e ExecHelm) ChartExists(chart string, opts *VendorOpts) (string, error) {
	// resolve the Chart relative to the caller
	callerDir := filepath.Dir(opts.CalledFrom)
	chart = filepath.Join(callerDir, chart)
	if _, err := os.Stat(chart); err != nil {
		return "", fmt.Errorf("helmTemplate: Failed to find a chart at '%s': %s. ", chart, err)
	}
